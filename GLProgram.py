...
class GLProgram:
    ...
    def genFragShaderSource(self):
        ...
        uniform bool {self.attribs["enableambient"]};
        uniform bool {self.attribs["enablediffuse"]};
        uniform bool {self.attribs["enablespecular"]};
        ...
        {{
            ...
            if ((renderingFlag >> 0 & 0x1) == 1){{
                ...
                vec3 norm = normalize(vNormal); // normal vector
                
                vec3 viewDirection = normalize({self.attribs["viewPosition"]} - vPos); // view direction
                vec4 result = vec4(0.0);
    
                for (int i = 0; i < MAX_LIGHT_NUM; i++) {{ // iterate through all lights
                    
                    if ({self.attribs["enableambient"]}) {{
                    ...
                    if ({self.attribs["enablespecular"]}) {{
                        if ((vR > 0.0)) {{
                    ...
                    if ({self.attribs["light"]}[i].spotOn) {{
                        float distance = length({self.attribs["light"]}[i].position - vPos); // distance between light and vertex
                        float radAttenX = {self.attribs["light"]}[i].spotRadialFactor.x * (pow(distance, 2)); // x component of radial attenuation
                        float radAttenY = {self.attribs["light"]}[i].spotRadialFactor.y * distance;          // y component of radial attenuation
                        float radAttenZ = {self.attribs["light"]}[i].spotRadialFactor.z;                     // z component of radial attenuation
                        
                        float radialAttenuation = 1.0 / (radAttenX + radAttenY + radAttenZ);                 // Compute radial attenuation
                        ...
                        vec4 diffuseComponent = diffuse * radialAttenuation * angularAttenuation;
                        vec4 specularComponent = specular * radialAttenuation * angularAttenuation;
            ...
            if ((renderingFlag >> 3 & 0x1) == 1){{
                ...
                vec3 normalColor = normalize(vNormal * 0.5 + 0.5); // Map the normal vector to RGB color space
