...
class Sketch(CanvasBase):
    ...
    def toggleLight(self, lightIndex):
        """
        Toggle the specified light in the current scene.
        :param lightIndex: Index of the light to toggle (0-based)
        """
        if lightIndex < 0 or lightIndex >= int(self.shaderProg.attribs["maxLightsNum"]):
            print(f"Invalid light index: {lightIndex + 1}")
            return

        # Get the current light state from the scene
        if not hasattr(self.scene, "lights"):
            print("Current scene does not support light toggling.")
            return

        self.scene.lights[lightIndex] = not self.scene.lights[lightIndex]
        self.shaderProg.setBool(f"light[{lightIndex}].enabled", self.scene.lights[lightIndex])
        state = "enabled" if self.scene.lights[lightIndex] else "disabled"
        print(f"Light {lightIndex + 1} is now {state}.")
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        if chr(keycode) in "aA":
            self.shaderProg.enableambient = not self.shaderProg.enableambient
            self.shaderProg.setBool("enableambient", self.shaderProg.enableambient)
            print(f"Ambient {'enabled' if self.shaderProg.enableambient else 'disabled'}")

        if chr(keycode) in "dD":
            self.shaderProg.enablediffuse = not self.shaderProg.enablediffuse
            self.shaderProg.setBool("enablediffuse", self.shaderProg.enablediffuse)
            print(f"Diffuse {'enabled' if self.shaderProg.enablediffuse else 'disabled'}")

        if chr(keycode) in "sS":
            self.shaderProg.enablespecular = not self.shaderProg.enablespecular
            self.shaderProg.setBool("enablespecular", self.shaderProg.enablespecular)
            print(f"Specular {'enabled' if self.shaderProg.enablespecular else 'disabled'}")
        if chr(keycode) in "zZ":
            self.switchScene(SceneOne(self.shaderProg))
            print("Switched to Scene 1 (Test Scene)")

        if chr(keycode) in "xX":
            self.switchScene(SceneTwo(self.shaderProg))
            print("Switched to Scene 2 (Custom Scene 2)")

        if chr(keycode) in "cC":
            self.switchScene(SceneThree(self.shaderProg))
            print("Switched to Scene 3 (Custom Scene 3)")
        if chr(keycode) in "nN":
            self.shaderProg.setFragmentShaderRouting("normal")
            print("Switched to Normal Rendering")


        if chr(keycode) in "123456789":
            lightIndex = int(chr(keycode)) - 1  
            self.toggleLight(lightIndex)
            print(f"Toggled light {lightIndex + 1} in the current scene.")
